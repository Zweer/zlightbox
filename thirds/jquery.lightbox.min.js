...
(function($, window, document, undefined) {
    var inspectNavigator = (function(neddle) {
        return function() {
            return neddle.search(arguments[0]);
        };
    ...
    if (inspectNavigator("mobile") > -1) {
        if (inspectNavigator("android") > -1 || inspectNavigator("googletv") > -1 || inspectNavigator("htc_flyer") > -1) {
            isMobile = true;
        }
    }

    if (inspectNavigator("opera") > -1) {
        if (inspectNavigator("mini") > -1 && inspectNavigator("mobi") > -1) {
            isMobile = true;
        }
    }

    if (inspectNavigator("iphone") > -1) {
        isMobile = true;
    }

    if (inspectNavigator("windows phone os 7") > -1) {
        isMobile = true;
    }
    ...
    $.extend({
        LightBoxObject: {
            ...
            create: function(options) {
                ...
                var container = $('<div class="' + name + ' ' + name + '-mode-image"><div class="' + name + '-border-top-left"></div><div class="' + name + '-border-top-middle"></div><div class="' + name + '-border-top-right"></div><a class="' + name + '-button-close" href="#close"><span>Close</span></a><div class="' + name + '-navigator"><a class="' + name + '-button-left" href="#"><span>Previous</span></a><a class="' + name + '-button-right" href="#"><span>Next</span></a></div><div class="' + name + '-buttons"><div class="' + name + '-buttons-init"></div><a class="' + name + '-button-left" href="#"><span>Previous</span></a><a class="' + name + '-button-max" href="#"><span>Maximize</span></a><div class="' + name + '-buttons-custom"></div><a class="' + name + '-button-right" href="#"><span>Next</span></a><div class="' + name + '-buttons-end"></div></div><div class="' + name + '-background"></div><div class="' + name + '-html"></div><div class="' + name + '-border-bottom-left"></div><div class="' + name + '-border-bottom-middle"></div><div class="' + name + '-border-bottom-right"></div></div>');
                var skelethon = this.esqueleto;
                this.overlay.create({
                    name: name,
                    ...
                    showDuration: (isMobile ? this.options.animation.show.duration / 2 : this.options.animation.show.duration),
                    closeDuration: (isMobile ? this.options.animation.close.duration / 2 : this.options.animation.close.duration)
                });
                skelethon.lightbox = container;
                skelethon.navigator = $('.' + name + '-navigator', container);
                skelethon.buttons.div = $('.' + name + '-buttons', container);
                skelethon.buttons.close = $('.' + name + '-button-close', container);
                skelethon.buttons.prev = $('.' + name + '-button-left', container);
                skelethon.buttons.max = $('.' + name + '-button-max', container);
                skelethon.buttons.next = $('.' + name + '-button-right', container);
                skelethon.buttons.custom = $('.' + name + '-buttons-custom', container);
                skelethon.background = $('.' + name + '-background', container);
                skelethon.html = $('.' + name + '-html', container);
                skelethon.move = $('<div class="' + name + '-move"></div>').css({
                    'position': 'absolute',
                    'z-index': this.options.style.zIndex,
                    'top': -999
                }).append(container);
                $('body').append(skelethon.move);
                this.win = $(window);
                this.addevents();
                return container;
            },
            addevents: function() {
                ...
                $window.bind('scroll', this.proxy(function() {
                    if (this.visible && this.move && !this.maximized && !isMobile) {
                        this.movebox();
                    }
                }));
            ...
            loadimage: function() {
                ...
                b.bind('complete', this.proxy(function() {
                    ...
                    b.append(a.image);
                    if (!compatibility.opacity) {
                        b.removeClass(c);
                    } else {
                        $(a.image).css("background-color", "rgba(255, 255, 255, 0)");
                        $(a.image).stop().css("opacity", 0).animate({
                            "opacity": 1
                        }, (isMobile ? this.options.animation.show.duration / 2 : this.options.animation.show.duration), function() {
                            b.removeClass(c);
                        });
                    }
            ...
            appendhtml: function(a, b, c, d) {
                ...
                h.bind('complete', function() {
                    ...
                    g.html.append(a);
                    if (d == "flash" && inspectNavigator("chrome") > -1) {
                        g.html.html(a);
                    }
                    h.unbind('complete');
            ...
            morph: function(d, f, g, h, i) {
                ...
                if (j[0] == 1 && j[1] < 8) {
                    var k = $.speed({
                        queue: i || false,
                        duration: (isMobile ? this.options.animation[g].duration / 2 : this.options.animation[g].duration),
            ...
            close: function(a) {
                ...
                if (ieVersion || !compatibility.opacity || isMobile) {
                    b.background.empty();
            ...
            link: function(b, optionsObj) {
                var $element = $(optionsObj.element);
                var e = this.getOptions($element);
                var rel = e.rel;
                var g = e.relent;
                var options = optionsObj.options;
                var j = [];
                $element.blur();
                if (optionsObj.gallery) {
                    j = optionsObj.gallery;
                    ...
                    var l = [];
                    var m = false;
                    $("a[" + g + "], area[" + g + "]", this.ownerDocument).filter("[" + g + "=\"" + rel + "\"]").each($.proxy(function(i, a) {
                        if ($element[0] === a) {
                ...
                $.lightbox(j, options, optionsObj.callback, $element);
                return false;
            },
            utils: {
                isEmpty: function(a) {
